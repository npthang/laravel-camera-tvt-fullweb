...
function upLoadImage($image, $path)
{
    ...
    if ($exten != 'jpg' && $exten != 'png' && $exten != 'gif' && $exten != 'jpeg') {
	    ...
	    $new_name = str_random(5) ."_". $old_name;
...
function showCategories($lang, $categories, $parent_id = 0, $char = '')
{
    ...
    if ($cate_child)
    {
        echo '<ul style="list-style-type: none; padding-left: 0px; text-align: left; font-size: 14px; line-height: 2">';
        foreach ($cate_child as $key => $item)
        {
            // Hiển thị tiêu đề chuyên mục
            echo '<li>'.$char.'<a target="_blank" href="/'.$lang.'/categories/'.$item->id.'" style="color: black;"><img width="20px"style="margin-right: 10px; border: solid 1px blue; border-radius: 50%;"src="/img/home/videocam.png">'.$item["name"].'</a>';

            // Tiếp tục đệ quy để tìm chuyên mục con của chuyên mục đang lặp
            showCategories($lang, $categories, $item['id'], $char.'|--');
            echo '</li>';
        }
        echo '</ul>';
    }
